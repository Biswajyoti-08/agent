"""
Enterprise WhatsApp AI OS — v5.1
Final Stable Production: Async, Multi-brand Routing, Atomic Dedup.
"""
...
async def should_escalate(text: str) -> bool:
    safe = text[:500]
    prompt = f"Analyze: '{safe}'. Reply 'ESCALATE' if user wants a manager, human, discount, or is angry. Otherwise 'AI_HANDLE'. Reply ONLY the word."
    try:
...
async def is_muted(sender: str) -> bool:
    ...
    if last and (datetime.utcnow() - last) < timedelta(minutes=30):
        await state_col.update_one({"phone_number": sender}, {"$set": {"last_human_interaction": datetime.utcnow()}})
        return True
    await state_col.update_one({"phone_number": sender}, {"$set": {"is_human_active": False}})
    return False

async def set_muted(sender: str, brand_id: str, muted: bool):
    await state_col.update_one(
        {"phone_number": sender},
        {"$set": {"brand_id": brand_id, "is_human_active": muted, "last_human_interaction": datetime.utcnow()}},
        upsert=True
    )

async def build_messages(brand: dict, sender: str, current_text: str) -> list:
    system = f"You are a Nike Concierge. Persona: {brand.get('persona')}. Goal: {brand.get('nudge_goal')} Rules: Max 3 short paragraphs. No competitors. Don't reveal you are AI."
    messages = [{"role": "system", "content": system}]
    history = await chat_col.find({"phone_number": sender}).sort("_id", -1).limit(6).to_list(length=6)
    history.reverse()
    for doc in history:
        if doc.get("user_msg"): messages.append({"role": "user", "content": doc["user_msg"]})
        if doc.get("ai_reply"): messages.append({"role": "assistant", "content": doc["ai_reply"]})
        if doc.get("manager_msg"): messages.append({"role": "assistant", "content": f"[Specialist said]: {doc['manager_msg']}"})
...
async def webhook(request: Request):
    ...
    if msg_id:
        ...
        if existing is not None: return Response(status_code=200)
    ...
    try:
        ...
        if is_outbound or is_manager:
            ...
            if customer:
                ...
                if user_text:
                    await chat_col.insert_one({"brand_id": brand_id, "phone_number": customer, "manager_msg": user_text, "timestamp": datetime.utcnow()})
        ...
        if user_text and await should_escalate(user_text):
            ...
            await asyncio.gather(send_whatsapp(mgr_phone, alert), send_whatsapp(sender, "I'm connecting you with a Nike specialist right away! 🙌"))
            return {"status": "escalated"}

        # ── 3. Mute check ──
        if await is_muted(sender):
            if user_text:
                await chat_col.insert_one({"brand_id": brand_id, "phone_number": sender, "user_msg": user_text, "muted": True, "timestamp": datetime.utcnow()})
            return {"status": "muted"}
        ...
        if location:
            ...
            if store:
                ...
                reply = f"📍 *Nearest Nike Store*\n\n*{store['store_name']}*\n{store.get('address', '')}\n\n🗺 Directions: {maps}\n\nJust Do It"
                await asyncio.gather(send_whatsapp(sender, reply), send_whatsapp(mgr_phone, f"🚨 *NEW LEAD*: {sender} is visiting {store['store_name']}"))
            if not user_text: return {"status": "location_handled"}

        # ── 5. AI Engine ──
        if not user_text: return Response(status_code=200)
        messages = await build_messages(brand, sender, user_text)
        completion = await groq.chat.completions.create(messages=messages, model="llama-3.1-8b-instant", max_tokens=300)
        ai_reply = completion.choices[0].message.content
        await asyncio.gather(
            chat_col.insert_one({"brand_id": brand_id, "phone_number": sender, "user_msg": user_text, "ai_reply": ai_reply, "timestamp": datetime.utcnow()}),
...
@app.get("/")
async def health():
    return {"status": "Enterprise AI OS — live", "version": "5.1"}